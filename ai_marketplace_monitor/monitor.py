...
class MarketplaceMonitor:
    ...
    def __init__(
        ...
        self.config_files = (
            ...
            + ([amm_personal_config] if amm_personal_config.exists() else [])
    ...
    def search_item(
        ...
        for listing in marketplace.search(item_config):
            ...
            # Optional AI pacing delay between model calls.
            _ai_delay = float(os.environ.get("AIMM_AI_DELAY_SECONDS", "0") or "0")
            if _ai_delay > 0:
        ...
        if new_listings:
            _aimm_pf = os.environ.get("AIMM_PRINT_FOUND", "1").strip().lower()
            if _aimm_pf not in ("0", "false", "no", "off"):
                ...
                print("\n".join(_aimm_lines), file=sys.stderr, flush=True)
            counter.increment(
