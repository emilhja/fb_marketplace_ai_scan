...
def main():
    ...
    with psycopg.connect(db_url) as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE l.availability ILIKE ANY(ARRAY['såld', 'borttagen']);
                """)
    ...
    if not total_urls:
        print("No inactive listings found in the database.")
        sys.exit(0)

    print(f"Found {total_urls} inactive listings.")
    ...
    logger = logging.getLogger("rescrape_inactive")
    ...
    print("Done rescraping and evaluating inactive listings.")
